...
int main() {
	...
	example::fill_random_data(in, tuples_count, 20);
	 ...
	 * Decompress
	 */
	auto   decompressed_buffer_size = alp::AlpApiUtils::align_value<size_t, alp::config::VECTOR_SIZE>(tuples_count);
	double decompressed[decompressed_buffer_size];
	auto   decompressor = alp::AlpDecompressor();
