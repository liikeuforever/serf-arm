...
int main() {
	...
	example::fill_random_data(in, tuples_count / 2, 2); // Half of the data has a limited precision
	example::fill_random_data(
	 ...
	 * Decompress
	 */
	size_t decompressed_buffer_size = alp::AlpApiUtils::align_value<size_t, alp::config::VECTOR_SIZE>(tuples_count);
