...
double calculate_alp_compression_size(std::vector<alp_bench::VectorMetadata>& vector_metadata) {
	...
	for (auto& metadata : vector_metadata) {
		...
		avg_bits_per_value = avg_bits_per_value + (static_cast<double>(metadata.exceptions_count) *
		                                           (alp::exception_size + alp::exception_position_size) / VECTOR_SIZE);
	}
...
TEST_F(alp_test, test_alp_on_whole_datasets) {
	...
	for (auto& dataset : alp_bench::alp_dataset) {
		...
		if (dataset.is_too_large) { continue; }
		...
		for (size_t i = 0; i < tuples_count; i++) {
			...
			alp::decode(unffor_arr, stt.fac, stt.exp, dec_dbl_arr);
		...
		if (alp_bench::results.find(dataset.name) != alp_bench::results.end()){ // To avoid error when tested dataset is not found on results
