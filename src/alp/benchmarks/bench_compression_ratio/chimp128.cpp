...
/*
 * Chimp overhead per vector in a hypothetic file format = leading_zero_block_count + size_of_data_block + start_of_data;
	...
	for (auto& dataset : alp_bench::alp_dataset) {
		...
		for (size_t i = 0; i < tuples_count; i++) {
            ...
            */
            ...
            compressed_data_size += (alp_bench::AlignValue(bytes_used_by_data) + flag_bytes + bytes_used_by_leading_zero_blocks + bytes_used_by_packed_data_blocks_count) * 8;
