...
struct ChimpCompressionState {
	...
	ChimpCompressionState() : previous_leading_zeros(NumericLimits<uint8_t>::Maximum()) {
		previous_value = 0;
	}
...
public:
    ...
    static constexpr uint8_t SIGNIFICANT_BITS_SIZE = 6; // The amount needed to store the maximum number of significant bits (0-63)
...
public:
	ChimpDecompressionState() : reference_value(0), first(true) {
		ResetZeros();
	}
...
public:
	...
	static inline CHIMP_TYPE DecompressValue(ChimpConstants::Flags flag, uint8_t leading_zeros[],
	                                         uint32_t &leading_zero_index, DecompressState &state) {
		CHIMP_TYPE result;
		switch (flag) {
