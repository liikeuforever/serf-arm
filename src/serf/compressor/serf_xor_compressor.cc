...
SerfXORCompressor::SerfXORCompressor(int capacity, double max_diff, long adjust_digit) : max_diff_(
    max_diff), adjust_digit_(adjust_digit) {
  output_buffer_ =
      std::make_unique<OutputBitStream>(std::floor(((capacity + 1) * 8 + capacity / 8 + 1) * 1.2));
...
void SerfXORCompressor::AddValue(double v) {
  ...
  if (__builtin_expect(
      std::abs(Double::LongBitsToDouble(stored_val_) - adjust_digit_ - v) > max_diff_, false)) {
