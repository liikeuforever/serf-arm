...
NetSerfXORCompressor::NetSerfXORCompressor(int capacity, double max_diff): kBlockSize(capacity), kMaxDiff(max_diff) {
...
Array<uint8_t> NetSerfXORCompressor::Compress(double v) {
  ...
  if (std::abs(Double::LongBitsToDouble(stored_val_) - v) > kMaxDiff) {
