...
SerfQtCompressor::SerfQtCompressor(int block_size, double max_diff) : max_diff_(max_diff * 0.999){
    ...
    compressed_size_in_bits_ += output_bit_stream_->WriteLong(
            Double::DoubleToLongBits(max_diff_), 64);
}
...
void SerfQtCompressor::AddValue(double v) {
    ...
    compressed_size_in_bits_ += EliasDeltaCodec::Encode(
...
Array<uint8_t> SerfQtCompressor::GetBytes() {
    Array<uint8_t> ret = output_bit_stream_->GetBuffer(std::ceil
            (static_cast<double>(stored_compressed_size_in_bits_) / 8.0));
