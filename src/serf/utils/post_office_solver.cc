...
PostOfficeSolver::InitRoundAndRepresentation(Array<int> &distribution,
    ...
    for (int z = 0; z <= max_z; ++z) {
        ...
        PostOfficeResult por = PostOfficeSolver::BuildPostOffice(
                distribution, num, total_count_and_non_zeros_counts[1],
...
PostOfficeResult
PostOfficeSolver::BuildPostOffice(Array<int> &arr, int num, int non_zeros_count,
        ...
        for (int j = std::max(1, num + i - arr.length());
            ...
            if (i > 1 && j == 1) {
                ...
                for (int k = j - 1; k <= i - 1; ++k) {
                    if (arr[k] == 0 && k > 0 || pre_non_zeros_count[k] < j ||
