...
int EliasDeltaCodec::Encode(int64_t number,
    ...
    if (number <= 11) {
        len = 1 + static_cast<int>(std::floor(kLogTable[number] /
                                              kLogTable[2]));
    } else {
        len = 1 + static_cast<int>(std::floor(std::log(number) / kLogTable[2]));
    }
    if (len <= 11) {
        length_of_len = static_cast<int>(std::floor(
                kLogTable[len] / kLogTable[2]));
    } else {
        length_of_len = static_cast<int>(std::floor(
                std::log(len) / kLogTable[2]));
    }
    int total_len = length_of_len + length_of_len + len;
    if (total_len <= 64) {
        compressed_size_in_bits += static_cast<int>(
                output_bit_stream_ptr->writeLong(
                        ((int64_t) len << (len - 1)) |
                        (number & ~(0xffffffffffffffffL << (len - 1))),
                        total_len));
    } else {
        compressed_size_in_bits += static_cast<int>(
                output_bit_stream_ptr->writeInt(
                        0, length_of_len));
        compressed_size_in_bits += static_cast<int>(
                output_bit_stream_ptr->writeInt(
                        len, length_of_len + 1));
