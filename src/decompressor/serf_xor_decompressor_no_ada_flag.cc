#include "serf_xor_decompressor_no_ada_flag.h"

std::vector<double> SerfXORDecompressorNoAdaFlag::Decompress(const Array<uint8_t> &bs) {
...
uint64_t SerfXORDecompressorNoAdaFlag::ReadValueEqualWinFalse() {
...
void SerfXORDecompressorNoAdaFlag::UpdateFlagAndPositionsIfNeeded() {
...
void SerfXORDecompressorNoAdaFlag::UpdateLeadingRepresentation() {
...
void SerfXORDecompressorNoAdaFlag::UpdateTrailingRepresentation() {
  int num = static_cast<int>(input_bit_stream_.ReadInt(5));
  if (num == 0) {
    num = 32;
  }
