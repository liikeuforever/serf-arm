...
inline double decode_value(const int64_t encoded_value, const uint8_t factor, const uint8_t exponent) {
	const double decoded_value = encoded_value * fact_arr[factor] * frac_arr[exponent];
	return decoded_value;
}

inline void decode(const int64_t* encoded_integers, const uint8_t fac_idx, const uint8_t exp_idx, double* output) {
