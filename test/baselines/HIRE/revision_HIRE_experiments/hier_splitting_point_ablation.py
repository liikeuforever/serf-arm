...
class HierarchicalSketch():                                                  
    ...
    def encode(self, data, size):
        ...
        # print(list(zip(hierarchy, residuals)))
        # print('ye')
        if size:
            return list(zip(residuals, level_sizes))
        return list(zip(residuals, hierarchy))
    ...
    - sketch: List[(h, r)]
    """
    def pack(self, sketch, size):
        # print(sketch)
        vectors = []
        i = 0
        for res, abl in sketch:
            # print("level", i)
            i+= 1
            vector = np.concatenate([np.array([res]), abl])
...
class MultivariateHierarchical(CompressionAlgorithm):
    ...
    def compress(self, size):
        ...
        for j in range(self.p):
            ...
            # generate encodings
            en = self.sketch.encode(vector, size)
            # print(en)
            # print('ye2')

            # pack each encoding
            arrays.append(self.sketch.pack(en, size))
