...
TEST(TestLZ77, CorrectnessTest) {
    ...
    for (const auto &dataSet: dataSetList) {
        ...
        while ((originalData = ReadBlock(dataSetInputStream)).size() == BLOCK_SIZE) {
            ...
            int compression_output_len = fastlz_compress_level(2, originalData.data(), BLOCK_SIZE * sizeof(double), compression_output);
            fastlz_decompress(compression_output, compression_output_len, decompression_output, BLOCK_SIZE * sizeof(double));
            for (int i = 0; i < BLOCK_SIZE; ++i) {
                EXPECT_TRUE(originalData[i] - decompression_output[i] == 0);
            }
